...
class TestGraph:
    ...
    def test_add_edge_bidirectional(self, simple_graph: RoadGraph):
        assert simple_graph.has_edge("A", "B")
        assert simple_graph.has_edge("B", "A")  # bidirectional

    def test_add_edge_auto_distance(self):
        g = RoadGraph()
    ...
    def test_remove_node(self, simple_graph: RoadGraph):
    ...
    def test_remove_node_missing(self, simple_graph: RoadGraph):
        with pytest.raises(KeyError):
            simple_graph.remove_node("MISSING")

    def test_remove_edge(self, simple_graph: RoadGraph):
        simple_graph.remove_edge("A", "B")
        assert not simple_graph.has_edge("A", "B")
        assert simple_graph.has_edge("B", "A")  # reverse still exists

    def test_remove_edge_missing(self, simple_graph: RoadGraph):
        with pytest.raises(KeyError):
            simple_graph.remove_edge("A", "D")  # no direct edge

    def test_get_neighbors(self, simple_graph: RoadGraph):
        neighbors = simple_graph.get_neighbors("A")
        dst_ids = {e.dst for e in neighbors}
        assert "B" in dst_ids
        assert "C" in dst_ids

    def test_node_count(self, simple_graph: RoadGraph):
        assert simple_graph.node_count == 4

    def test_edge_count(self, simple_graph: RoadGraph):
        # 4 bidirectional edges = 8 directed edges
        assert simple_graph.edge_count == 8

    def test_subgraph(self, simple_graph: RoadGraph):
...
class TestRouting:
    def test_astar_finds_path(self, simple_graph: RoadGraph):
    ...
    def test_dijkstra_finds_path(self, simple_graph: RoadGraph):
        result = dijkstra(simple_graph, "A", "D")
        assert result is not None
        assert result.path[0] == "A"
        assert result.path[-1] == "D"

    def test_astar_same_distance_as_dijkstra(self, simple_graph: RoadGraph):
    ...
    def test_astar_explores_fewer_nodes(self, city_graph: RoadGraph):
    ...
    def test_same_start_end(self, simple_graph: RoadGraph):
        result = astar(simple_graph, "A", "A")
        assert result is not None
        assert result.path == ["A"]
        assert result.distance_km == 0.0

    def test_invalid_node(self, simple_graph: RoadGraph):
        assert astar(simple_graph, "MISSING", "A") is None
        assert dijkstra(simple_graph, "A", "MISSING") is None

    def test_path_through_grid(self, city_graph: RoadGraph):
...
class TestRoutingTiles:
    ...
    def test_cross_tile_routing(self, city_graph: RoadGraph):
        tm = TileManager(city_graph, precision=2)
        all_keys = list({tm.node_tile_key(nid) for nid in city_graph.nodes})
        merged = tm.merge_tiles(all_keys)
...
class TestETA:
    def test_eta_basic(self, simple_graph: RoadGraph):
        result = astar(simple_graph, "A", "D")
        assert result is not None
    ...
    def test_eta_single_node(self, simple_graph: RoadGraph):
        eta = compute_eta(simple_graph, ["A"])
        assert eta.total_distance_km == 0.0
        assert eta.total_time_minutes == 0.0

    def test_eta_with_traffic(self, simple_graph: RoadGraph):
        result = astar(simple_graph, "A", "D")
        assert result is not None
    ...
    def test_traffic_multiplier_default(self):
        tm = TrafficModel()
        assert tm.get_multiplier("X", "Y") == 1.0

    def test_traffic_multiplier_set(self):
        tm = TrafficModel()
        tm.set_traffic("X", "Y", 3.0)
        assert tm.get_multiplier("X", "Y") == 3.0

    def test_traffic_invalid_multiplier(self):
        tm = TrafficModel()
    ...
    def test_congested_segments(self):
        tm = TrafficModel()
        tm.set_traffic("A", "B", 2.0)
        tm.set_traffic("C", "D", 1.5)
        assert len(tm.congested_segments) == 2

    def test_eta_average_speed(self, simple_graph: RoadGraph):
        result = astar(simple_graph, "A", "D")
        assert result is not None
        eta = compute_eta(simple_graph, result.path)
        assert eta.average_speed_kmh > 0

    def test_eta_invalid_path(self, simple_graph: RoadGraph):
        with pytest.raises(ValueError):
            compute_eta(simple_graph, ["A", "D"])  # no direct edge A->D

    def test_eta_segment_times(self, simple_graph: RoadGraph):
        result = astar(simple_graph, "A", "D")
        assert result is not None
        eta = compute_eta(simple_graph, result.path)
...
class TestGeocoding:
    def test_register_and_geocode(self):
    ...
    def test_reverse_geocode_tolerance(self):
        geo = GeocodingService()
        geo.register("Seoul Station", 37.554, 126.972)
    ...
    def test_reverse_geocode_out_of_range(self):
        geo = GeocodingService()
        geo.register("Seoul Station", 37.554, 126.972)
