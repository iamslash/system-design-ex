...
@pytest_asyncio.fixture
async def idempotency(redis_client):
...
@pytest_asyncio.fixture
async def wallet(redis_client):
    """Create a wallet service."""
    return WalletService(redis_client)


@pytest_asyncio.fixture
async def payment_service(redis_client, executor, idempotency, ledger, wallet):
...
@pytest_asyncio.fixture
async def failing_payment_service(redis_client, failing_executor, idempotency, ledger, wallet):
